    ...
    return new Promise(function (resolve, reject) {
        ...
        contents.replace(regex, function (match, $1, $2) {
            ...
            if (!fs.existsSync(absolute)) {
                absolute = path.join(opt.rootPath, $2);
            }
            ...
            var exist = _.findIndex(images, function (img) {
                return img.absolute === absolute && img.match === match;
            });
            // 如果不存在,才添加进去
            if (exist === -1) {
                images.push({
...
module.exports = function (opt) {
    opt = _.extend({margin: 2, rootPath: ''}, opt);
    var stream = through.obj(function (vinyl, encoding, cb) {
        ...
        var self = this;
        parse(vinyl, opt).then(function (images) {
